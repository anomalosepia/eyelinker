...
The lines correspond to "MSG" lines in the imported ASC file. Since these messages can be anything, `read.asc()` leaves them unparsed: if you're interested in subsetting them or extracting data from them, you can parse them youself using packages such as `stringr` or with the built-in `grep`/`grepl` functions.
